<script lang="ts">
	...
	$: cashIncome = data.incomes.find((i) => i.method === 'point-of-sale')?.amount ?? 0;
	...
	$: needsJustification = data.cashDeltaJustificationMandatory || Math.abs(cashDelta) > 0.01;
	$: canSubmit = needsJustification ? justification.trim().length > 0 : true;
</script>
		...
		<div class="space-y-4">
			...
			<div>
				...
				{#each data.incomes as income}
					<p class="ml-4">
						• {income.method}: {income.amount.toFixed(2)}
						{income.currency}
					</p>
				{/each}
	...
	<form method="POST" class="space-y-6">
			...
			<div class="bg-yellow-50 border border-yellow-300 rounded-lg p-4">
				<label for="justification" class="form-label text-yellow-900">
					<strong class="inline-flex items-center gap-1">
						...
						Justification {data.cashDeltaJustificationMandatory ? 'required (always)' : 'required'}
					</strong>
					<br />
		...
		<div class="flex gap-3">
			<button
				...
				disabled={!canSubmit}
				class="btn"
				class:btn-green={canSubmit && !needsJustification}
				class:btn-orange={canSubmit && needsJustification}
