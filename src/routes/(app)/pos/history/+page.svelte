			...
			<table class="admin-table">
				<thead class="admin-table-header">
					<tr>
						...
						<th class="admin-table-th text-left">Employee</th>
				...
				<tbody>
					{#each data.sessions as session}
						<tr>
							...
							<td class="admin-table-td text-sm">
								<div>
									<div class="font-medium">{getUserDisplay(session.openedBy)}</div>
									{#if session.closedBy && session.closedBy.login !== session.openedBy.login}
										<div class="text-xs text-gray-500">→ {getUserDisplay(session.closedBy)}</div>
									{/if}
								</div>
							</td>
